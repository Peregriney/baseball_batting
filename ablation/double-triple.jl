...
function Double(i::Int)::Float64
    ...
    ld = Float64(lambda)
    if flag == "uniform"
        return  playersData[penultimate_index, 6]  - (ld)*(playersData[penultimate_index, 6])
    end
...
function Triple(i::Int)::Float64
    ...
    ld = Float64(lambda)
    if flag == "uniform"
        return playersData[penultimate_index, 7]  + (ld)*(playersData[penultimate_index, 6])
    end
...
elseif length(ARGS) == NUM_BATTERS + 3
    ...
    global lambda = parse(Float64, ARGS[NUM_BATTERS+3])
end
...
ludf.lambda = Vector{Float64}(undef, nrow(ludf))
...
for i in 1:1 #nrow(ludf)
    ...
    if haskey(seenLineups, lineup_str)
        ...
        (lineupval, k, v, ld) = seenLineups[lineup_str]
        ...
        if length(lineup) == 9
          ...
          ludf.team[i] = team
          ludf.lambda[i] = lambda
          seenLineups[lineup_str] = (e, join(sortedkeys, ", "), join(sortedvals, ", "), lambda)
        end
    end
end
println(ludf.score2)
...
CSV.write("v2_probs-double-hr-2024-halfway.csv", ludf,append=true)
